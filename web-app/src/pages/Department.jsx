      ...
      if (confirm("This Department will be permanently deleted?")) {
        ...
        if (response) {
          ...
          setDevices((prevDevices) =>
            prevDevices.filter((device) => device._id !== rowId)
          );
        }
      ...
      if (response.status == 201) {
        setDevices((prevDevices) =>
          prevDevices.map((device) =>
            device._id === deviceData._id ? response.data : device
          )
        );
        handleCloseModal();
  ...
  return (
    <MiniDrawer>
      ...
      >
        <h1>Departments</h1>
        <Box>
          {hasEntityPermission("create", "department", permissions) && (
            <ScrollDialog
              title={"Department"}
              form={<Form submitHandler={handleSubmitForm} data={data} />}
              data={data}
              modalOpen={modalOpen}
