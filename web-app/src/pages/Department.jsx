...
export const Department = () => {
    ...
    const handleDeleteClick = async (rowId) => {
        ...
        if (rowId !== "") {
            if (confirm("This Department will be permanently deleted?")) {
                const response = await departmentService.deleteDepartments(rowId);
                if (response) {
    ...
    const handleSubmitForm = async (deviceData) => {
        // console.log('Department data:', deviceData);
        if (deviceData._id) {
            const response = await departmentService.updateDepartment(deviceData._id, deviceData);
            ...
            const response = await departmentService.createDepartment(deviceDataWithoutId);
            if (response.status == 201) {
    ...
    return (
        <MiniDrawer>
            ...
            <h1>Departments</h1>
                <Box>
                    {hasEntityPermission('create', 'department', permissions) && <ScrollDialog title={"Department"} form={<Form submitHandler={handleSubmitForm} data={data} />} data={data} modalOpen={modalOpen} handleOpenModal={handleOpenModal} handleCloseModal={handleCloseModal} />}
                    <Button sx={{ mb: 2, ml: 1 }} onClick={fetchDepartments} variant="contained" startIcon={<RefreshIcon />}>Refresh</Button>
                </Box>
            </Box>
            <LinearProgress sx={{ display: loading == false && "none" }} />
            {devices && <ControledDataGrid entity={'department'} tableData={devices} columns={columns} handleEditClick={handleEditClick} handleDeleteClick={handleDeleteClick} action={true} />}
        </MiniDrawer>
    );
};
