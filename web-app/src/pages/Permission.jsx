...
const Permission = () => {
  ...
  // Generate the default permissions template (includes all entities, e.g. 'store')
  const permissionsObject = generatePermissionsObject(entities, roles);

  // Merge server permissions with defaults so newly added entities also appear
      ...
      <MiniDrawer>
        ...
        <Grid container spacing={2}>
          ...
          <Grid item xs={8}>
            ...
            {mergedPermission && Object.keys(mergedPermission).length > 0 ? (
              <IndeterminateCheckboxList data={mergedPermission} />
            ) : (
...
const Form = ({ submitHandler, data }) => {
  ...
  useEffect(() => {
    if (formValues.department === "") {
      ...
      setFilteredUsers(
        users.filter((user) => user.department === formValues.department)
      );
    }
  ...
  const extractDepartments = (data) => {
    const uniqueDepartments = Array.from(
      new Set(data.map((user) => user.department_name))
    );
    setDepartments([
      { name: "All", id: "" },
      ...uniqueDepartments.map((dept) => ({
        name: dept,
        id: data.find((user) => user.department_name === dept).department,
      })),
    ]);
  };
